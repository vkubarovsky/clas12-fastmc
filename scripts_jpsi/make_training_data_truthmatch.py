...
def build_parser():
    ...
    p.add_argument("--electron_match", choices=["pidonly", "pidonly_ft", "exactpid", "truth"],
                   default="pidonly",
                   help="How to match the electron: 'pidonly' = reconstructed "
                        ...
                        "'pidonly_ft' = like pidonly, but the SCATTERED electron "
                        "(first MC pid==11) is matched in the Forward Tagger "
                        "(status 1xxx) while the decay leptons (e+/e-) stay in FD "
                        "(J/psi: e' in FT, J/psi->e+e- in FD); "
    ...
    p.add_argument("--max_electron_theta", type=float, default=180.0,
                   help="Drop events with generated electron theta above this (deg) "
                        "(e.g. 7.0 for an FT electron)")
...
def main():
    ...
    elif args.electron_match == "pidonly_ft":
        print("Electron match: scattered e- (pid==11) in FT (status 1xxx), nearest in p, no window;")
        print("                decay leptons (pid==+-11) in FD, nearest in p, no window")
        print("Hadrons restricted to FD only (no CD)")
    ...
    print(f"Drop gen electron theta < {args.min_electron_theta} "
          f"or > {args.max_electron_theta} deg")
    ...
    header_lines = [
        ...
        f"{' (FD only)' if args.electron_match == 'pidonly_ft' else ''}; "
        f"electron {'PID==11 in FD nearest in p' if args.electron_match=='pidonly' else 'scattered PID==11 in FT nearest in p, decay leptons PID==+-11 in FD nearest in p' if args.electron_match=='pidonly_ft' else 'exact-PID+kinematic window' if args.electron_match=='exactpid' else 'TruthMatch quality>'+str(electron_quality)}; "
        f"species from MC::Particle.pid; "
        f"gen electron theta in [{args.min_electron_theta}, {args.max_electron_theta}] deg",
            ...
            for batch in chain:
                ...
                for i in range(n_ev):
                    ...
                    if e_theta is not None and (e_theta < args.min_electron_theta
                                                or e_theta > args.max_electron_theta):
                        n_skipped_theta += 1
                        continue
                    ...
                    # index of the scattered electron (first MC pid==11)
                    scat_idx = None
                    for mc in mc_info:
                        if mc['pid'] == 11:
                            scat_idx = mc['index']
                    ...
                    for mc in mc_info:
                        ...
                        if is_electron and args.electron_match in ("pidonly", "pidonly_ft"):
                            # reconstructed lepton (exact pid), nearest in p, no window.
                            # pidonly: all leptons in FD.
                            # pidonly_ft: scattered e- in FT, decay leptons in FD.
                            want_det = 3 if (args.electron_match == "pidonly_ft"
                                             and k == scat_idx) else 1
                            best_j = -1; best_dp = 1e9
                            for j in range(nrec):
                                if int(r_pid[i][j]) != mc['pid']:
                                    continue
                                if det_from_status(r_st[i][j]) != want_det:
                                    continue
                            ...
                            if k in truth and truth[k][1] > qmin:
                                ...
                                if args.electron_match == "pidonly_ft" and det != 1:
                                    # J/psi: hadrons in FD ONLY (no CD) — a match
                                    # outside the FD counts as not detected
                                    rp = rt = rph = rvz = -999.0
                                    rec_pid = 0; det = 0; found = False
                                else:
                                    rp, rt, rph = kinematics_from_xyz(
                    ...
                    # electron gating flag
                    # pidonly_ft: scattered e- found in FT
                    # otherwise:  any e+/e- found in FD
                    e_gated = False
                    if args.electron_match == "pidonly_ft":
                        if (scat_idx is not None
                                and particles[scat_idx]['status'] > 0
                                and particles[scat_idx]['det'] == 3):
                            e_gated = True
                    else:
                        for p in particles:
    ...
    print(f"Skipped gen e- theta outside [{args.min_electron_theta}, "
          f"{args.max_electron_theta}]: {n_skipped_theta}")
