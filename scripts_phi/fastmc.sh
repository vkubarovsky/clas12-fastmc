...
PY_SCRIPTS_DEFAULT=/home/vpk/fastmc/scripts_phi
...
if [ $# -lt 1 ]; then
    echo "Usage: $0 {build-dat|build-cuts|train|validate|all|clean} [options]" >&2
    exit 2
fi
CMD="$1"; shift
...
do_train() {
    ...
    echo "[train] training 4 MLPs into $MODELS_DIR (threads=$OMP_NUM_THREADS)"
    # electron — full file
...
do_clean() {
    echo "[clean] removing contents of $OUT/{dat,cuts,models,plots,report}"
    rm -f "$DAT_DIR"/*.dat
    rm -f "$CUTS_DIR"/*.json "$CUTS_DIR"/*.pdf
    rm -f "$MODELS_DIR"/*.pt
    rm -f "$PLOTS_DIR"/*.pdf
    rm -f "$REPORT_DIR"/*.log "$REPORT_DIR"/params.txt "$REPORT_DIR"/launch.sh
    echo "[clean] done"
}
