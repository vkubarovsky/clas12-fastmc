...
def build_cuts_for_particle(d, name, det_code, det_label, nbins, poly_deg_p, poly_deg_theta,
    ...
    Enforces the physical FD / CD theta boundary at fd_cd_boundary degrees
    (default 35°): FD requires theta_gen <  boundary, CD requires theta_gen >= boundary.
    This avoids the status flag mislabeling near the boundary biasing the fit.
    """
    mask = d['status'] >= 1
