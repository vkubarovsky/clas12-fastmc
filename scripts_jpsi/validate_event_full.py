...
def make_pdf(pdf_path, label, gemc, fmc, gen_arrays, npart, names, reaction,
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        fmc_all_mask = fmc[names[0]]['accept'].copy()
        for nm in names[1:]:
            fmc_all_mask = fmc_all_mask & fmc[nm]['accept']
        ...
        # J/psi (FT electron) kinematic ranges
        fig, axes = plt.subplots(2, 5, figsize=(25, 10))
        for col, (var, vlab, rng, log_y) in enumerate([
            (Q2, "Q² (GeV²)",     (0, 0.25),    True),
            (xB, "xB",            (0, 0.016),   True),
            (W,  "W (GeV)",       (4.0, 4.6),   False),
            (-t, "-t (GeV²)",     (0, 8),       True),
            (Mh, "M(e+e-) (GeV)", (3.05, 3.15), False),
        ]):
        ...
        # ── Page 5: M(e+e-) + MM ──
        # J/psi: meson daughters are e+ (index 2) and e- (index 3)
        m_e = PDG_MASS[11]; m_pr = PDG_MASS[2212]; m_dau = PDG_MASS[11]
        M_MESON = 3.097
        ...
        # GEMC M(e+e-) and MM on all-4 events
        ig = gemc_all_idx
        gemc_Mh = compute_invariant_mass(
            gemc_p_rec[3][ig], gemc_theta_rec[3][ig], gemc_phi_rec[3][ig], m_dau,
            gemc_p_rec[2][ig], gemc_theta_rec[2][ig], gemc_phi_rec[2][ig], m_dau)
        gemc_MM = MM_arr(
        ...
        f_pe, f_te, f_phie, f_vze = rec_at(names[0], fmc_all_idx)
        f_pp, f_tp, f_phip, f_vzp = rec_at(names[1], fmc_all_idx)
        f_pkp, f_tkp, f_phikp, _  = rec_at(names[2], fmc_all_idx)
        f_pkm, f_tkm, f_phikm, _  = rec_at(names[3], fmc_all_idx)

        fmc_Mh = compute_invariant_mass(
            f_pkp, f_tkp, f_phikp, m_dau, f_pkm, f_tkm, f_phikm, m_dau)
        ...
        axes[1, 1].axvline(M_MESON, color="red", ls="--")
...
if __name__ == "__main__":
    ...
    # Make names unique — J/psi (epe+e-) has two e-: index 0 is the scattered
    # electron (FT, keeps the name 'e-' and the gating role), index 3 the decay
    # electron (FD), renamed 'e-d'.  Model files must match (e-.pt, e-d.pt).
    seen = {}
    for k in range(npart):
        if names[k] in seen:
            names[k] = names[k] + 'd'
